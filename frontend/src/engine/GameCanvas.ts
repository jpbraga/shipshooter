...
export default class GameCanvas {
  ...
  private onEffect = (type: string, x: number, y: number, data?: unknown): void => {
    if (!this.particleSystem || !this.debrisSystem) return;
    ...
    switch (type) {
      ...
      case "hit":
        this.createHitParticles(x, y, Number(d["color"] || 0xffff00));
        break;
      case "bombFlash":
        this.createFlash(Number(d["alpha"] || 0.8));
        break;
    }
  };
  ...
  private renderPlayer(): void {
    ...
    this.playerGraphic.rotation = player.tiltAngle;
  ...
  private drawEnemyShip(graphic: Graphics, type: string, time: number, _health: number): void {
    ...
    switch (type) {
      ...
      case "minion_fighter":
        // Minion fighter - small fast ship
        graphic.moveTo(0, -15);
        graphic.lineTo(-12, 12);
        graphic.lineTo(0, 8);
        graphic.lineTo(12, 12);
        graphic.closePath();
        graphic.fill({ color: 0xff4444 });
        graphic.stroke({ color: 0xffaaaa, width: 1 });
        graphic.circle(0, 0, 4 * pulse);
        graphic.fill({ color: 0xffffff, alpha: 0.8 });
        break;
        
      case "minion_bomber":
        // Minion bomber - round bomber
        graphic.circle(0, 0, 18);
        graphic.fill({ color: 0xaa3333 });
        graphic.stroke({ color: 0xff6666, width: 2 });
        graphic.circle(0, 5, 6);
        graphic.fill({ color: 0xff8800, alpha: pulse });
        break;
        break;
    }
  }
  ...
  private renderBoss(): void {
    ...
    this.bossGraphic.rotation = boss.tiltAngle;
    if (boss.shieldActive) {
      this.drawBossShield(boss);
    }
  ...
  private bossShieldGraphic: Graphics | null = null;
  private drawBossShield(boss: any): void {
    if (!this.entityContainer) return;
    if (!this.bossShieldGraphic) {
      this.bossShieldGraphic = new Graphics();
      this.entityContainer.addChild(this.bossShieldGraphic);
    }
    const shield = this.bossShieldGraphic;
    shield.clear();
    shield.x = boss.x;
    shield.y = boss.y;
    const shieldAlpha = boss.shieldHealth / boss.shieldMaxHealth;
    shield.circle(0, 0, boss.width / 2 + 20);
    shield.stroke({ color: 0x00ffff, alpha: shieldAlpha * 0.6, width: 4 });
    shield.circle(0, 0, boss.width / 2 + 15);
    shield.stroke({ color: 0x00aaff, alpha: shieldAlpha * 0.4, width: 2 });
  }
  ...
  createFlash(alpha: number = 0.8): void {
  ...
  createWreck(x: number, y: number, size: number = 20, color: number = 0x666666): void {
    ...
    this.debrisSystem.createWreck({ x, y, size, color, pieceCount: 5 });
  }
